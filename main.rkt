  ...
  (let parkour ((truba pipes))
    (if (null? truba)
        ...
        (if (or (check-collision-rectangles
                                              (make-posn (get-bird-x bird) (get-bird-y bird)) ; coltu stanga ??
                                              (make-posn (+ (get-bird-x bird) bird-width) (+ (get-bird-y bird) bird-height)) ;coltu dreapta jos??
                                              (make-posn (get-pipe-x (car truba)) 0) ;colt gap stanga ??
                                              (make-posn (+ (get-pipe-x (car truba)) pipe-width) (get-pipe-y (car truba))))
                                               ;colt gap dreapta jos??
                 
                       (check-collision-rectangles
                                              (make-posn (get-bird-x bird) (get-bird-y bird)) ; coltu stanga ??
                                              (make-posn (+ (get-bird-x bird) bird-width) (+ (get-bird-y bird) bird-height)) ;coltu dreapta jos??
                                              (make-posn (get-pipe-x (car truba)) ( + (get-pipe-y (car truba)) pipe-self-gap)) ;colt sus stanga ??
                                              (make-posn (+ (get-pipe-x (car truba)) pipe-width) ( - scene-height ground-height)))) ;colt dreapta jos??
...
(define get-variables 'here)
(define get-variables-gravity 'here)
(define get-variables-momentum 'here)
(define get-variables-scroll-speed 'here)
